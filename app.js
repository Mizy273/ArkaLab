...
const els = {
  ...
  totalVolume: $("#totalVolume"), // Dipautkan ke kad Isipadu dalam HTML
...
// LOGIK BARU: INPUT DIBACA SEBAGAI ML, TOTAL BERAT (G) DIKIRA BERDASARKAN DENSITY
function updateStatsFromRows() {
  const rows = readIngredientRows();
  
  // Membaca jumlah input secara murni sebagai unit isi padu murni (ml)
  const totalMl = totalWeight(rows); 
  
  // Nilai purata ketumpatan fizikal bagi cecair pati wangi + alkohol
  const KETUMPATAN_PERFUME = 0.83; 
  
  // Formula: Isipadu (ml) x Ketumpatan = Anggaran Berat Komposisi (g)
  const totalG = totalMl * KETUMPATAN_PERFUME;

  // Paparkan berat hasil penukaran formula fizik
  els.totalWeight.textContent = `${totalG.toFixed(2)} g`;
  
  // Paparkan isipadu yang tepat mengikut nilai input yang dimasukkan (Contoh: 5.70 ml)
  if (els.totalVolume) {
    els.totalVolume.textContent = `${totalMl.toFixed(2)} ml`;
  }
  
  els.ingredientCount.textContent = rows.filter((item) => item.name || parseFloat(item.weight)).length;

  // Kira pecahan peratusan (%) berdasarkan total Isipadu (ml)
  [...els.ingredientRows.querySelectorAll("tr")].forEach((row) => {
    const volumeInput = parseFloat(row.querySelector("[data-field='weight']").value) || 0;
    const percentCell = row.querySelector(".percent-cell");
    percentCell.textContent = totalMl > 0 ? `${((volumeInput / totalMl) * 100).toFixed(2)}%` : "0.00%";
  });
}
...
function renderForm(recipe) {
  ...
  if (isEmpty) {
    ...
    if (els.totalVolume) els.totalVolume.textContent = "0.00 ml"; 
    return;
  }
